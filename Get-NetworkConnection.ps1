function Get-NetworkConnection {
    ...
    .NOTES
	This code was edited by @hadar0x to include a timestamp for each extracted connection


    Original Author: Lee Christensen (@tifkin_)
...
function Get-Tcp4Connections
{
    ...
    try
    {
        ...
        for($i=0; $i -lt $OwnerModuleTable.NumEntries; $i++)
        {
            ...
            $TimeStamp = [System.DateTime]::FromFileTime($TcpRow.CreateTimestamp)
...
function Get-Udp4Connections
{
    ...
    try
    {
        ...
        for($i=0; $i -lt $OwnerModuleTable.NumEntries; $i++)
        {
            ...
            $LocalPort = $PortBytes[0]*256 + $PortBytes[1]
            $TimeStamp = [System.DateTime]::FromFileTime($UdpRow.CreateTimestamp)
